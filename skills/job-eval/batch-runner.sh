...
# job-eval — standalone orchestrator for claude -p workers
# Reads batch-input.tsv, delegates each offer to a claude -p worker,
# tracks state in batch-state.tsv for resumability.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
BATCH_DATA="$SCRIPT_DIR/batch-data"
INPUT_FILE="$BATCH_DATA/batch-input.tsv"
STATE_FILE="$BATCH_DATA/batch-state.tsv"
PROMPT_FILE="$SCRIPT_DIR/batch-prompt.md"
LOGS_DIR="$BATCH_DATA/logs"
# Notion handles tracker
LOCK_FILE="$BATCH_DATA/batch-runner.pid"
STATE_LOCK_DIR="$BATCH_DATA/.batch-state.lock"
...
usage() {
  cat <<'USAGE'
job-eval — process job offers in batch via claude -p workers
...
Options:
  ...
  --from-notion        Query Notion for "Scanned" jobs and generate batch-input.tsv
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --from-notion) FROM_NOTION=true; shift ;;
...
# Generate batch-input.tsv from Notion "Scanned" jobs
generate_input_from_notion() {
  ...
  if (( count == 0 )); then
    echo "No 'Scanned' jobs found in Notion."
    exit 0
  fi

  echo "Found $count scanned jobs. Generating batch-input.tsv..."

  # Write TSV with page_id column
  printf 'id\turl\tsource\tnotes\tpage_id\n' > "$INPUT_FILE"
  echo "$json" | python3 -c "
import sys, json
...
for i, job in enumerate(data['jobs'], 1):
    ...
    print(f\"{i}\t{url}\tNotion\t{company} — {role}\t{page_id}\")
" >> "$INPUT_FILE"

  local generated
  generated=$(tail -n +2 "$INPUT_FILE" | grep -c '[^[:space:]]' 2>/dev/null || true)
  echo "Generated $INPUT_FILE with $generated offers (skipped jobs without URLs)"
}
...
next_report_num_unlocked() {
  local max_num=0
  # Reports now live in Notion — only check state file for assigned numbers
  # (No local reports directory to scan)
  if [[ -f "$STATE_FILE" ]]; then
...
process_offer() {
  local id="$1" url="$2" source="$3" notes="$4" page_id="${5:-}"
  ...
  local resolved_prompt="$BATCH_DATA/.resolved-prompt-${id}.md"
  # Escape sed delimiter characters in variables to prevent substitution breakage
  local esc_url esc_jd_file esc_report_num esc_date esc_id esc_page_id
  esc_url="${url//\\/\\\\}"
  esc_url="${esc_url//|/\\|}"
  esc_jd_file="${jd_file//\\/\\\\}"
...
# Post-batch (Notion handles merge — no local merge needed)
merge_tracker() {
  echo ""
  echo "=== Evaluations written directly to Notion ==="
  echo "No local merge needed — each worker pushed to Notion via db_applications.py"
}
...
main() {
  ...
  local -a pending_page_ids=()

  while IFS=$'\t' read -r id url source notes page_id; do
